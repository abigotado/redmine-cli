...
func TestIssueCreateInvalidFieldsFailsBeforeNetwork(t *testing.T) {
	registry := &memoryRegistry{values: map[string]profile.Profile{"work": {Name: "work", BaseURL: "https://redmine.test"}}}
	app := &App{registry: registry, store: panicStore{}, stdin: panicReader{}, stdout: &bytes.Buffer{}, stderr: &bytes.Buffer{}, newRedmine: func(profile.Profile, auth.Credential, *slog.Logger) (redmineReader, error) { panic("network") }}
	code := app.Run(context.Background(), app.NewRootCommand(), []string{"issues", "create", "--profile", "work", "--project-id", "1", "--subject", "x", "--yes", "--fields", "typo"})
